...
step_2() {
    ...
    until oc apply -f "${PROJECT_DIR}"/configs/02; do : ; done
}

step_3() {
    logbanner "Install kserve dependencies"
    until oc apply -f "${PROJECT_DIR}"/configs/03; do : ; done
}

step_4() {
    logbanner "Install RHOAI operator"
    until oc apply -f "${PROJECT_DIR}"/configs/04; do : ; done
}
...
step_7() {
    ...
    until oc apply -f "${PROJECT_DIR}"/configs/07; do : ; done
}

step_8() {
    logbanner "Run sample gpu application"
    until oc apply -f "${PROJECT_DIR}"/configs/08; do : ; done
}

step_9() {
    logbanner "Configure gpu dashboards"
    until oc apply -f "${PROJECT_DIR}"/configs/09; do : ; done
}

step_10() {
    logbanner "Configure gpu sharing method"
    until oc apply -f "${PROJECT_DIR}"/configs/10; do : ; done
}

step_11() {
    logbanner "Configure distributed workloads"
    until oc apply -f "${PROJECT_DIR}"/configs/11; do : ; done
}

step_12() {
    logbanner "Configure codeflare operator"
    until oc apply -f "${PROJECT_DIR}"/configs/12; do : ; done
}

step_13() {
    logbanner "Configure rhoai"
    until oc apply -f "${PROJECT_DIR}"/configs/13; do : ; done
}
